...
type oauthCachedCredentials struct {
	Auth struct{ Token *oauth2.Token }
}
...
func (cache *oauthYamlCredentialsCache) Get(audience string) *oauth2.Token {
	...
	return cachedCredentials.Auth.Token
}
...
func (cache *oauthYamlCredentialsCache) put(audience string, token *oauth2.Token) {
	cache.lock.Lock()
	defer cache.lock.Unlock()
	cache.audiences[audience] = &oauthCachedCredentials{
		Auth: struct{ Token *oauth2.Token }{Token: token},
	}
}
